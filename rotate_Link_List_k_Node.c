...
To rotate the linked list, we need to change 
1. next of kth node to NULL, 
2. next of last node to previous head node, and finally change 
3. head to (k+1)th node. 

So we need to get hold of three nodes: kth node, (k+1)th node and last node.
Traverse the list from beginning and stop at kth node. Store pointer to kth node. We can get (k+1)th node 
using kthNode->next. Keep traversing till end and store pointer to last node also. Finally, change pointers as stated above.
