...
All these operations occur simultaneously.  It's guaranteed that there won't be any overlap in replacement: for example, S = "abc", indexes = [0, 1],
...
class Solution:
    def findReplaceString(self, S, indexes, sources, targets):
        ...
        :rtype: str
        """
        for i, f, t in sorted(zip(indexes, sources, targets), reverse=True):
            if S[i : i + len(f)] == f:
                S = S[:i] + t + S[i+len(f): ]
        return S
