...
class Solution(object):
    def findCelebrity(self, n):
        ...
        :rtype: int
        """
        # we declare someone celebrity here
        celeb = 0
        for i in range(1, n):
            if knows(i, celeb) and not knows(celeb, i):
                # other person knows celeb and celeb doesn't know other
                # then continue, and not change celeb
                continue
            else:
                celeb = i

        for j in range(celeb):
            # just make sure other doesn't know celeb then return false
            # or celeb in-case knows someone then too
