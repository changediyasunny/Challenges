...
group representing 16 bits. The groups are separated by colons (":"). For example, the address 2001:0db8:85a3:0000:0000:8a2e:0370:7334 is a valid one. Also, we could omit some
leading zeros among four hexadecimal digits and some low-case characters in the
...
class Solution(object):
    def validIPAddress(self, IP):
        ...
        def is_ipv4(strs):
            try:
                # "01.01.01.01" >> pass it with str(int(strs)) == strs
                return str(int(strs)) == strs and 0 <= int(strs) <= 255
            except:
                return False

        def is_ipv6(strs):
            if len(strs) > 4:
                # "2001:0db8:85a3:00000:0:8A2E:0370:7334" >> to pass this one.
                return False
            try:
                # "1081:db8:85a3:01:-0:8A2E:0370:7334" >> to pass this one
                return int(strs, 16) >= 0 and strs[0] != '-'
            except:
                return False
