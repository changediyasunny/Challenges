...
with fixed warm radius to warm all the houses. Now, you are given positions of houses
and heaters on a horizontal line, find out minimum radius of heaters so that all houses
could be covered by those heaters. So, your input will be the positions of houses and heaters
seperately, and your expected output will be the minimum radius standard of heaters.
...
class Solution(object):
    def findRadius(self, houses, heaters):
        ...
        for h in sorted(houses):
            while h >= heaters[i+1]:
                i += 1
            # leftHeater = h - heaters[i]
            # rightHeater = heaters[i+1] - h
